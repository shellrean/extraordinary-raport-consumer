<template>
  <div class="bg-white w-full lg:w-1/2 py-4 px-4 rounded-lg shadow border border-gray-300">
    ...
    <div class="mb-4">
      ...
      <div class="flex">
        <select v-model="academic_active.id" class="w-full pl-4 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-blue-300">
          <option :value="academicID" v-for="(academic, index) in academics" :key="index">{{ academic.name }} Semester {{ academic.semester }}</option>
        </select> 
      </div>
    </div>
    <button @click="" class="py-1 px-3 rounded-md bg-green-400 border-2 border-green-500 hover:shadow-xl disabled:opacity-50 text-white font-medium">Simpan</button> 
  </div>
</template>
<script>
